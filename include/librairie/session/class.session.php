...
//----------------------------------------------------------------------
	...
	*/	
	function write($arg_str_session_id, $arg_str_session_data) 
	{
		...
		mcrypt_generic_deinit($td);
		mcrypt_module_close($td);
		if(COOKIE_DOMAIN) setcookie(session_name(), session_id(), time()+3600, "/", (COOKIE_DOMAIN ? "." . COOKIE_DOMAIN : NULL));
		setcookie($arg_str_session_id, $cypher, time()+3600, "/", (COOKIE_DOMAIN ? "." . COOKIE_DOMAIN : NULL));
		ob_end_flush();
		return true;
	}
//----------------------------------------------------------------------
